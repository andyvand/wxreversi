...
wxBEGIN_EVENT_TABLE(ReversiFrame, wxFrame)
    ...
    EVT_MENU(ID_HelpContents,  ReversiFrame::OnHelpContents)
              ...
              wxDefaultPosition, wxSize(480, 540))
{
    ...
    helpMenu->Append(ID_HelpContents, "&Reversi Help\tF1");
...
void ReversiFrame::OnHelpContents(wxCommandEvent&)
{
#ifdef __APPLE__
    // .help bundle lives in Contents/Resources; helper opens Help Viewer.
    ShowMacHelpBook(nullptr);
#elif defined(__WXMSW__)
    // wxreversi.chm is deployed alongside the executable.
    wxFileName exeFn(wxStandardPaths::Get().GetExecutablePath());
    wxString chm = exeFn.GetPath() + wxFILE_SEP_PATH + "wxreversi.chm";
    if (!wxFileExists(chm))
        chm = "wxreversi.chm";
    if (!wxLaunchDefaultApplication(chm)) {
        wxMessageBox("Unable to open wxreversi.chm.",
                     "Reversi", wxOK | wxICON_ERROR, this);
    }
#else
    // Linux fallback: open the bundled HTML index if present, else nothing.
    wxString idx = wxStandardPaths::Get().GetResourcesDir() +
                   "/wxreversi.help/Contents/Resources/English.lproj/index.html";
    if (wxFileExists(idx))
        wxLaunchDefaultBrowser("file://" + idx);
    else
        wxMessageBox("Help is not installed.",
